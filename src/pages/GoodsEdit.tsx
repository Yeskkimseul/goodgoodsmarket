...
const GoodsEdit = () => {
    ...
    useEffect(() => {
        ...
        if (stored) {
            const goodsList: Goods[] = JSON.parse(stored);
            const found = goodsList.find(item => item.id === id);
            if (found) {
                setTitle(found.title);
                setDescription(found.description);
                setImageUrls(Array.isArray(found.imageUrl) ? found.imageUrl : [found.imageUrl]);
                setCategory(found.category);
                setPrice(found.price);
                setOptions(found.options || []);
                setSelectedCondition(found.options?.[0] || "");
                setSelectedItem(found.options?.[1] || "");
                setSelectedDelivery(found.options?.[2] || "");
                setOptionLabel((found.options || []).join(" · ") || "옵션 추가");
                setIsNegotiable(found.isExchangeable || false);
            }
        }
    }, [id]);
    ...
    const handleOptionComplete = () => {
        ...
        setTimeout(() => setShowOptionPopup(false), 0);
        setOptionLabel(allOptions.join(" · "));
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const stored = localStorage.getItem('goodsList');
        let goodsList: Goods[] = stored ? JSON.parse(stored) : [];
        const updatedGoods: Goods = {
            ...
            imageUrl: imageUrls,
            likes: 0, // 필요시 기존 값 유지
            createdAt: new Date().toISOString(), // 필요시 기존 값 유지
            views: 0, // 필요시 기존 값 유지
            options,
            sellerimgUrl: '/images/users/profile.png',
            sellerName: '뱃지가좋아',
            sellerTrust: 70,
            isExchangeable: isNegotiable,
        } as Goods;
